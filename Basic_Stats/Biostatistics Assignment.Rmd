...
There is a clear visual distinction between cuckoo egg dimensions and the species of host parent. Figure 1 and 2 clearly demonstrate this, with eggs being cared for by wren species being noticably smaller than the other host-parents. 

```{r cuckoo egg length boxplot, echo=FALSE}
```

```{r cuckoo egg breadth boxplot, echo=FALSE, fig.cap=paste("Diagram of cuckoo egg length and breadth corresponding to species of host-parent"), fig.width = 12, fig.height = 4}
...
boxplots
```
The largest variation in size occurs in the Meadow Pipit, with the other host-parent species have a relatively proportional size.
